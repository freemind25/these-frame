...
import { exchangeCode, getBoxProfile } from '@/lib/box-drive'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Get user profile
    const profile = await getBoxProfile(tokens.access_token)

    // Upsert connection in DB
    const existing = await db.cloudDriveConnection.findFirst({ where: { provider: 'box' } })
